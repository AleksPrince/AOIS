...
class BooleanAnalyzer:
    ...
    def _print_minimization(self):
        ...
        print("\n  А. Расчетный метод:")
        res, stages = self.glue_minimizer.minimize(f)
        for s in stages[-5:]:
            print(f"    {s}")
        print(f"\n    РЕЗУЛЬТАТ: {res}")

        print("\n  Б. Расчетно-табличный метод:")
        res, stages, table = self.table_minimizer.minimize(f)
        if table:
        ...
        print("\n  В. Карта Карно:")
        res, kmap = self.karnaugh_minimizer.minimize(f)
        for line in kmap.split('\n'):
            print(f"    {line}")
        print(f"\n    РЕЗУЛЬТАТ: {res}")