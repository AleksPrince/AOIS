...
class KarnaughMinimizer:
    ...
    def minimize(self, function: LogicFunction) -> Tuple[str, str]:
        ...
        # Добавляем оба результата в карту
        map_str += f"\n\nДНФ: {result_dnf}"
        map_str += f"\nКНФ: {result_cnf}"
    ...
    def _minimize_2var_cnf(self, function: LogicFunction) -> str:
        ...
        kmap = {}
        for bits, res in function.truth_matrix:
            kmap[(bits[0], bits[1])] = 1 - res  # инвертируем
        ...
        for group in groups:
            term = self._term_from_group_2var(group, variables)
            if term and term not in terms:
                # Для КНФ нужно инвертировать литералы
                term = self._invert_term(term, variables)
                terms.append(term)

        if not terms:
            return "1"
        return " ∧ ".join([f"({t})" if '|' not in t else t for t in terms])
    ...
    def _minimize_3var_cnf(self, function: LogicFunction) -> str:
        ...
        kmap = {}
        for bits, res in function.truth_matrix:
            kmap[(bits[0], bits[1], bits[2])] = 1 - res  # работаем с нулями
    ...
    def _minimize_5var_dnf(self, function: LogicFunction) -> str:
        ...
        return " ∨ ".join(sorted(terms)) if terms else "0"

    def _minimize_5var_cnf(self, function: LogicFunction) -> str:
        # Для КНФ используем инверсию (работаем с нулями)
        gray = [(0, 0), (0, 1), (1, 1), (1, 0)]
        kmap = {}
        for bits, res in function.truth_matrix:
            kmap[(bits[0], bits[1], bits[2], bits[3], bits[4])] = 1 - res
        ...
        for group in groups0:
            term = self._term_from_group_4var(group, rest_vars)
            if term and term != "1":
                term = self._invert_term(term, rest_vars)
                found = False
                for g1 in groups1:
                    t1 = self._term_from_group_4var(g1, rest_vars)
                    t1_inv = self._invert_term(t1, rest_vars)
                    if t1_inv == term:
                        found = True
                        break
                if found:
        ...
        for group in groups1:
            ...
            if term and term != "1":
                ...
                for g0 in groups0:
                    t0 = self._term_from_group_4var(g0, rest_vars)
                    t0_inv = self._invert_term(t0, rest_vars)
                    if t0_inv == term:
                        found = True
                        break
                if not found:
        ...
        if not terms:
            return "1"
        return " ∧ ".join([f"({t})" if '|' in t else t for t in sorted(terms)])
    ...
    def _invert_term(self, term: str, vars_list: List[str]) -> str:
        """Инвертирует терм для получения КНФ"""
        if term == "1":
            return "0"
        if term == "0":
            return "1"
