...
class GlueMinimizer:
    ...
    def minimize(self, function: LogicFunction) -> Tuple[str, List[str]]:
        """Минимизация - возвращает лучший результат (ДНФ или КНФ)"""
        stages = []
        stages.append("=" * 60)
        ...
        # Минимизируем по ДНФ (по единицам)
        stages.append("\n--- МИНИМИЗАЦИЯ ПО ДНФ (ПО ЕДИНИЦАМ) ---")
        result_dnf, stages_dnf = self._minimize_by_dnf(sdnf, function)
        stages.extend(stages_dnf)

        # Минимизируем по КНФ (по нулям) - без вывода этапов
        result_cnf, _ = self._minimize_by_cnf(sknf, function)

        # Выбираем лучший результат (по количеству символов)
    ...
    def _covers_cnf(self, term: str, bits: Tuple[int, ...], variables: List[str]) -> bool:
        """Проверка, покрывает ли терм КНФ набор (т.е. обращается в 1 на нулевом наборе)"""
        literals = self.term_proc.split_into_literals(term)
        vmap = dict(zip(variables, bits))
        for lit in literals:
            if lit.startswith('!'):
